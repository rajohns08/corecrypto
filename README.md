...
Building for iOS simulators + iOS devices
------
This was tested on Xcode 13.3.1, macOS 12.3.1.
...
In the app consuming the built libs, make sure to bring in the headers (can be generated by building the `corecrypto_headers` target), set Header Search Paths build setting, and set Other Linker Flags build setting to determine which lib to use for which architecture:

![Alt text](img/structure.png?raw=true "Title")

![Alt text](img/headers.png?raw=true "Title")

![Alt text](img/linker.png?raw=true "Title")
