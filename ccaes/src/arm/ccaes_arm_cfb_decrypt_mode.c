...
extern void ccaes_cfb_decrypt_vng_vector(const ccecb_ctx *ctx, uint8_t *iv, int nblocks, const uint8_t *in, uint8_t *out);

static int ccaes_cfb_decrypt_vng(cccfb_ctx *key, size_t nbytes, const void *in, void *out)
{
  ...
  if (nbytes>=16) {  
...
#if defined(_ARM_ARCH_7) && CC_KERNEL
    /*
    ...
    if ((((int)in&0x03)==0) && (((int)out&0x03)==0)) {  // both in and out are 4-byte aligned
            ccaes_cfb_decrypt_vng_vector(ecb_key, iv, (int)nblocks, ct, pt);
            nblocks <<= 4;
            ct += nblocks;
            pt += nblocks;
            nbytes -= nblocks;
        ...
        while (nblocks) {
            cc_memcpy((void*)aligned_in, ct, CCAES_BLOCK_SIZE);
            ccaes_cfb_decrypt_vng_vector(ecb_key, iv, 1, (const uint8_t*)aligned_in, (uint8_t*)aligned_out);
            cc_memcpy(pt, (void*)aligned_out, CCAES_BLOCK_SIZE);
            ct += CCAES_BLOCK_SIZE;
            pt += CCAES_BLOCK_SIZE;
...
#if CC_ACCELERATECRYPTO
            if (AccelerateCrypto_AES_encrypt((const void *) iv, (void *) pad, (const AccelerateCrypto_AES_ctx*) ecb_key)) {
#else
            if (ccaes_arm_encrypt(iv, pad, (const ccaes_arm_encrypt_ctx*) ecb_key)) {
#endif
                return -1;
            }
            pad_len = 0;
