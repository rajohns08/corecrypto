...
static int test_ccprime_rabin_miller_small()
{
...
static int test_ccprime_rabin_miller_zero_rounds()
{
    cc_unit p[] = { 0x9c75b };
...
static int test_ccprime_rabin_miller_vectors()
{
    for (unsigned i = 0; i < vectors_n; i++) {
        ...
        CC_DECL_WORKSPACE_OR_FAIL(ws, CC_MAX_EVAL(CCPRIME_RABIN_MILLER_INIT_WORKSPACE_N(n),
                                                  CCPRIME_RABIN_MILLER_ITERATION_WORKSPACE_N(n)));
        CC_DECL_BP_WS(ws, bp);

        ccprime_mr_decl_n(n, mr);
        ccprime_rabin_miller_init(ws, mr, n, p->units);
        is(ccprime_rabin_miller_iteration(ws, mr, b->units), tv->r, "MR iteration failed");
...
int ccprime_rabin_miller_tests(TM_UNUSED int argc, TM_UNUSED char *const *argv)
{
    int kat_tests = (int)vectors_n + 1;
    plan_tests(14 + kat_tests);

    test_ccprime_rabin_miller_small();
    test_ccprime_rabin_miller_zero_rounds();
    test_ccprime_rabin_miller_known_primes();

    is(test_ccprime_rabin_miller_vectors(), CCERR_OK, "MR KATs failed");
